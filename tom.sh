...
echo"now going to directory /root/apache-tomcat-8.5.73/bin"

cd apache-tomcat-8.5.73/bin;

echo "Create soft link to start/stop tomcat service"

ln -s /root/apache-tomcat-8.5.73/bin/startup.sh /etc/init.d/tomcatstart
ln -s /root/apache-tomcat-8.5.73/bin/shutdown.sh /etc/init.d/tomcatstop
...
mv -f manager-METAINF apache-tomcat-8.5.73/webapps/manager/META-INF/context.xml

cd ~
wget https://raw.githubusercontent.com/hitman-045/tomcatinstall/master/host-manager-METAINF-context.xml
mv -f host-manager-METAINF-context.xml  apache-tomcat-8.5.73/webapps/host-manager/META-INF/context.xml
...
mv -f tomcat-users.xml apache-tomcat-8.5.73/conf/tomcat-users.xml
